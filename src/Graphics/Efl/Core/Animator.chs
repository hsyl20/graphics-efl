...
-- | Add a new animation
addAnimation :: (Double -> Double) -> Maybe Double -> (Double -> IO Bool) -> IO Animator
addAnimation modif Nothing f = addAnimator (f . modif)
addAnimation modif (Just d) f = addTimedAnimator d (f . modif)

-- | Add an animation with linear steps
addAnimationLinear :: Maybe Double -> (Double -> IO Bool) -> IO Animator
addAnimationLinear = addAnimation id

-- | Add an animation with bouncing steps
addAnimationBounce :: Maybe Double -> (Double -> IO Bool) -> IO Animator
addAnimationBounce = addAnimation g
   where g x | x <= 0.5  = 2.0 * x
             | otherwise = 2.0 * (1.0 - x)

-- | Add an animation with sinusoidal steps
addAnimationSinusoidal :: Int -> Maybe Double -> (Double -> IO Bool) -> IO Animator
